bbox <- function(xlim,ylim){
  st_polygon(list(cbind(c(xlim[1],xlim[1],xlim[2],xlim[2],xlim[1]),
                        c(ylim[1],ylim[2],ylim[2],ylim[1],ylim[1])))) %>%
    st_sfc() %>% 
    st_set_crs("+proj=longlat +datum=WGS84") 
}

geom_north <- function(xscale = 1, yscale = xscale, lon, lat,...){
  geom_polygon(data = tibble(x = c(.01,0,-.01,0,.01)* xscale + lon, 
                             y = c(.02,.03,.02,.05,.02) * yscale + lat),
               aes(x = x, y = y),...)
}
...
scaleBar <- function(lon, lat, distanceLon, distanceLat, distanceLegend, dist.unit = "km", rec.fill = "white", rec.colour = "black", rec2.fill = "black", rec2.colour = "black", legend.colour = "black", legend.size = 3, orientation = TRUE, arrow.length = 500, arrow.distance = 300, arrow.North.size = 6){
  ...
  # Legend
  scaleBarLegend <- annotate("text", label = paste(laScaleBar$legend[,"text"], dist.unit, sep=""), x = laScaleBar$legend[,"long"], y = laScaleBar$legend[,"lat"], size = 1.4*legend.size, colour = legend.colour,fontface = "bold")
